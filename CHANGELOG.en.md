...
- **New: Typewriter Mode** — New "Typewriter Mode" toggle (default off) in the Tabs settings section. When enabled, the cursor line is kept in the middle band of the page (range centering / dead-zone scrolling): the viewport is split into a top eighth, a middle 3/4, and a bottom eighth; the cursor line is left untouched while it sits in the middle dead zone (12.5%–87.5%), and when it falls into the top or bottom eighth it is scrolled to the top of the dead zone (12.5% of the viewport) — far fewer scroll operations than precise centering, reducing visual fatigue. Scrolling is skipped while the mouse is pressed/dragging and triggers only after release, so drag-selection is not disturbed. Lines outside the dead zone (top/bottom eighth) are dimmed per the "Outside Dead-Zone Opacity" sub-setting (0-100 slider, default 50, shown only while the mode is on), while lines inside the dead zone and the current line stay bright. Implementation: two viewport-fixed translucent veils over the top/bottom eighth (`position: absolute` + CSS percentage heights), inherently bound to the viewport — the dim boundary always matches the viewport during scroll/click, with no coordinate math or cache lag. New command "Toggle Typewriter Mode" (`mdrazor-toggle-typewriter`) can be bound to a hotkey and stays bidirectionally in sync with the settings toggle (the settings switch and sub-setting visibility refresh immediately after a command toggle)
