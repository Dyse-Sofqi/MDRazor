...
- **New: "Allow Blank Area at Document Top" sub-toggle for Typewriter Mode** — Default on. When enabled, fixed blank space of (viewport height − line height) / 2 is reserved at the top of the editor so the cursor can sit at the page center even on the very first line (otherwise first-line centering is clamped by the scroll top edge); when disabled, the original behavior is restored. Implementation: toggles a class on `.cm-editor` and sets a CSS variable, applied by the stylesheet to `.cm-editor`'s own `padding-top` — the blank sits outside the scroll container, is always pinned at the top of the editor (independent of scroll position, never scrolls out of view), and sits above the inline title so the title stays flush with the first line. Shown only while the mode is on
