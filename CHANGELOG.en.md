...
- **Symbol boundary hint: the `|` in the tooltip now aligns with the caret** (it no longer sits off to the right of the cursor) — previously the whole tooltip landed to the right of the caret, and the `|` inside it was separated from the caret by a distance that varied with the tooltip's content (10.6–39.2px measured), so the two appeared to move independently as the cursor moved through a marker. The cause is CM6's positioning: `writeMeasure` aligns the tooltip's **left edge** to the caret's x (`left = pos.left + offset.x`, where `offset` is the official hook CM6 provides for exactly this), while the `|` sits a further `d = border + left padding + left text width + | advance / 2` to the right. The hint now publishes `-d` through `TooltipView.offset`; `d` is measured after the tooltip is mounted (`mount()` measures the distance from the tooltip's left edge to the `|` glyph's centre), so it adapts to any font and size without maintaining a character-width formula. **Result: as the caret moves left/right inside a hidden marker, the `|` stays pinned to the caret and only the tooltip's outline grows and shrinks.** Measured in a real CM6 run: deviation was **0.00px at all 9 caret positions** (10.6–39.2px before the change).
- **The alignment offset is re-measured after zoom / font / pane-size changes** — the offset is measured at the current font size, so it goes stale when that changes. Such changes set CM6's `geometryChanged`, and the hint is now force-rebuilt in those updates to re-measure (reusing the existing path built for the space-visualization toggle). Scrolling does **not** set that flag (every assignment site of CM6's `UpdateFlag.Geometry` was checked), so the tooltip is not rebuilt on every frame.
- **Fix: the symbol boundary hint was nearly invisible in dark mode** — the tooltip's fill is `--background-primary`, **exactly the same colour as the editor canvas**, so all of its depth came from `box-shadow: 0 2px 8px rgba(0,0,0,0.15)`. A black shadow's amplitude is proportional to the backdrop's brightness: the same 15% black removes about 38 code values from white (an obvious soft dark halo) but only about 4 from `#1C1C1C`, which the 8px blur then spreads into invisibility. One red herring was ruled out: **the border was not the cause** — dark `#333` against the canvas is 1.35:1, actually slightly *better* than light `#e4e4e4` against white at 1.27:1. Dark mode now gets its own override rule: the fill is derived from the canvas colour and the text colour via `color-mix` (guaranteeing it is a step lighter than the canvas), the border moves to `--background-modifier-border-focus`, and the shadow is heavier (a 1px outer dark edge plus 16px/60%). Measured across three themes: border contrast 1.31–1.35:1 → **2.29:1**, fill separation 1.19–1.21:1.
- **The dark-mode "raised surface" no longer depends on `--background-secondary`** — in dark themes that variable is usually one step lighter than the canvas (Obsidian's default ramp has base-20 > base-00, and Ethereal follows it), but that is a convention, not a contract: Blue Topaz defines it **darker** than the canvas (`#151515` vs `#202020`), which would turn the tooltip into a sunken panel there. The fill is therefore derived from the canvas itself with `color-mix(in srgb, var(--text-normal) 8%, var(--background-primary))`, keeping the "lighter" direction consistent across all three themes. The selector is `.theme-dark .mdrazor-boundary-hint.mdrazor-boundary-hint`, whose specificity `(0,3,0)` beats the original rule without `!important`, leaving light mode untouched.
- **Known limitation** — the `|` alignment is computed for LTR only; RTL paragraphs keep the previous behaviour (in RTL CM6 aligns the tooltip's right edge to the anchor, a mirrored formula under which the same negative offset would push the wrong way).

**2.6.3** (2026-09-15)

- **Fix: plugin review failure `Disabling 'obsidianmd/prefer-file-manager-trash-file' is not allowed`** — the previous release kept its "force the system trash" behaviour by adding an `eslint-disable` line to `vault.trash(file, true)`; the reviewer's `eslint-plugin-obsidianmd` 0.4.x adds `eslint-comments/no-restricted-disable`, which **forbids disabling any obsidianmd rule**, so the review was rejected outright (the local 0.3.0 has no such rule, which is why local lint was green while the review was red). It now uses the official entry point `fileManager.trashFile(file)`, which honours the user's **Settings → Files and links → Deleted files** preference (system trash / vault `.trash` / permanent). To compensate for a user who picked "permanently delete", the cleanup dialog's description now states where the deletion method comes from. **Behaviour change: recoverability is no longer forced.**
- **`minAppVersion` 1.1.0 → 1.6.6** — `FileManager.trashFile()` requires Obsidian ≥1.6.6, and the reviewer's 0.4.x `no-unsupported-api` rule cross-checks the API's introduction version against the manifest's declared minimum, reporting an error otherwise. The README's Obsidian badge now reads `^1.6.6`.
- **16 `createElement` call sites now use Obsidian's DOM helpers** — the native `createElement('div' | 'span')` calls in `cursor-boundary-hint.ts`, `whitespace-visible.ts`, `dir-file-count.ts`, `vertical-tabs.ts` and `status-bar-enhancer.ts` are all `createDiv()` / `createSpan()` now: detached nodes use the globally exported helpers, nodes with a known parent are created on that parent, and the `appendChild` calls that became redundant were removed. With 0.4.x adding `prefer-create-el` to the recommended set, every one of these sites reported a warning.
- **Lint toolchain aligned with the review environment** — `eslint-plugin-obsidianmd` 0.3.0 → 0.4.2 (the root cause of "green locally, red in review": the old local version had neither `no-restricted-disable` nor `prefer-create-el` / `no-unsupported-api` enabled); `eslint.config.mts` migrated from the deprecated `tseslint.config` to ESLint core's `defineConfig`.
- **Class-name typo fixed** — the status-bar workspace name `mdrazror-workspace-name` → `mdrazor-workspace-name` (both the creation and the query site; `styles.css` never referenced it).
- **Known leftover** — `obsidianmd/settings-tab/prefer-setting-definitions` (warning): the settings tab does not yet implement Obsidian 1.13+'s declarative settings API `getSettingDefinitions()`, so its settings do not appear in settings search. That API is not in the current `obsidian` typings (1.12.3) and adopting it is a feature-level rewrite, deferred to a later release.

**2.6.2** (2026-09-15)

- **Idempotent release workflow (ends a run of failed runs)** — the `release.yml` workflow ran `gh release create` on tag pushes, but this project's release flow creates the release through the REST API first and only then creates the tag, so that step always failed on the existing same-named release and turned the whole run red (2.5.15 / 2.5.16 / 2.6.0 / 2.6.1 in a row; the failing step was always `Create release` while `Build plugin` succeeded). It now probes with `gh release view` first: if the release exists it prints the tag / draft flag / asset list and skips creation, otherwise it creates it. Both release paths (API-first, and push-a-tag-and-let-the-workflow-publish) now converge; when a release already exists its **assets are left untouched**, so a CI build can never silently replace the three files whose digests were already verified.
- **Workspace switcher menu now uses Obsidian's DOM helpers** — the menu and its items are built with `statusBarEl.createDiv({ cls })` / `menuEl.createDiv({ cls })` instead of `doc.createElement('div')`, matching the official `prefer-create-el` rule (prefer `createEl` / `createDiv` and friends). The host elements belong to the correct document, so the menu is still created in the right `document` inside pop-out windows; the finished menu is then moved to `document.body`, with zero visual or behavioural change.

**2.6.1** (2026-09-15)

- **Fix: `Calls to EditorView.update are not allowed while an update is in progress` on reopening a document, which also killed position restore** — when the position-persistence `ViewPlugin.update()` detected a whole-document load it synchronously called `view.dispatch()` to restore the cursor. But `EditorView.update()` sets its internal `updateState` to `Updating` *before* it calls each plugin's `update()`, so that dispatch is guaranteed to throw; worse, when CM6 catches a plugin exception it calls `destroy()` and `deactivate()` on the plugin instance — so this was not just a console error, the cursor/scroll tracking and restore for that editor stopped working **permanently** until the view was recreated. The decision now stays inside `update()` while the dispatch is deferred to `queueMicrotask` (the update cycle is synchronous, so the microtask runs after the current synchronous work and before the browser paints — the cursor never flashes at the mapped position first), with duplicate-schedule dedup and a destroy guard. It is triggered by whole-document replacement transactions (first load, another plugin formatting the whole file, reload after an external change), so vaults running Linter / obsidian-git are far more likely to hit it.
- **No more direct style assignment (Obsidian plugin review guidelines)** — visibility of status-bar items, left-ribbon buttons and settings sub-items moved from `el.style.display = ...` to `toggleClass('mdrazor-hidden', ...)` plus a `.mdrazor-hidden` rule in `styles.css`; the workspace-switcher menu's dynamic placement moved from inline `bottom` / `left` to `setCssProps()` publishing the `--mdrazor-menu-bottom` / `--mdrazor-menu-left` custom properties, with the positioning rule living in `styles.css`.
- **All remaining lint / review items cleared** — `instanceof HTMLElement` replaced by the cross-window safe `instanceOf(HTMLElement)`; an async listener passed to `addEventListener` now discards its Promise explicitly; the deprecated `workspace.activeLeaf` replaced by `getActiveViewOfType(MarkdownView)`; bare `setTimeout` replaced by `window.setTimeout` (pop-out window compatibility); the `any` used to walk the workspace split tree in `vertical-tabs` replaced by an explicit shape interface; `click-sync`'s `caretRangeFromPoint` fallback is kept through a narrowed type (it is the only caret API on Chromium < 128 / older Electron) instead of referencing the deprecated member directly. `npm run lint` and `tsc --noEmit` are both silent now.
- **Orphan image cleanup keeps its deletion method (deliberate deviation)** — it still uses `vault.trash(file, true)` to force the system trash instead of `fileManager.trashFile()`, which would honour the user's "permanently delete" preference; this is a one-shot bulk delete where a mis-ticked file is costly, so guaranteed recoverability wins. The reasoning is documented in the code.

**2.6.0** (2026-09-13)

- **Fix: clicking the lower half of a list row after a callout and dragging right selected the next row** — In live preview Obsidian renders a callout as a block widget (`inline-block`, baseline-aligned) whose anonymous line box hangs roughly half a line-height (10px measured) below the widget's own box, while CodeMirror's height map only measures the widget box — so every row after a callout sits 10px lower in the DOM than the height map claims. Clicks in the bottom band of those rows were mapped to the next row and a rightward drag selected the whole next row from the same column; rows before the callout were never affected. The widget box is now bottom-aligned to its line box and the gap is folded into the widget's own transparent padding (`vertical-align: bottom` + `padding-bottom: 10px`) — pixel-identical layout, height map and DOM geometry fully in sync.
- **Fix: drag end overwritten by the native mouseup re-select (click-sync hardening)** — Click-sync's per-move corrections only hold while the mouse is moving; when a non-empty selection exists before mousedown, CM6's native MouseSelection recomputes and re-dispatches the selection from the last mousemove position on mouseup, clobbering the corrected final selection. A final correction now runs on mouseup (gated on the drag having corrected at least once), so the visible final selection always matches the DOM truth, with zero intervention when the native result is already correct.
- **Investigation & compatibility note** — the gap is directly triggered by community copies of an old core rule (`.cm-callout { display: inline-block }`, once used to fix callout width); Obsidian 1.13+ no longer ships that rule. The plugin now compensates for it centrally, so such snippets can stay in place alongside this release unchanged (details in DEBUGLOG).

**2.5.16** (2026-09-11)

- **Fix: orphan image cleanup missed Canvas references and could delete canvas-only images** — The scanner only read `.md` notes (`extension === 'md'`), so `.canvas` files never entered reference collection and every image referenced from a canvas fell into the orphan set; combined with an all-checked-by-default dialog and a direct move to trash, one confirmation silently deleted in-use canvas assets. Reference sources now cover three groups: Markdown notes (body syntax + frontmatter properties), Canvas files (parsed as JSON) and other text carriers (`.base` / `.excalidraw` / `.html` / `.txt`).
- **Canvas references are parsed as JSON, not text regex** — A canvas is JSON, and its primary reference form `{"type":"file","file":"assets/a.png"}` keeps the path in a `file` field that no Markdown regex covers (only a hand-written `![[a.png]]` inside a text node gets caught by accident). The plugin now walks `nodes`: file nodes contribute `node.file`; text nodes feed `node.text` into the existing extractor; link nodes (external URLs) are ignored. If JSON parsing fails or the structure is unexpected it degrades to text regex plus a JSON path-field fallback, so a canvas corrupted by an external tool does not become a new blind spot.
- **Link resolution now uses the official API, and all strategies are unioned** — Hand-rolled `path.endsWith(...)` / `f.name === bareName` matching under-counts on duplicate filenames and relative paths, and under-counting is exactly what causes wrongful deletion. `metadataCache.getFirstLinkpathDest(ref, sourcePath)` is now tried first (handling relative paths, subfolders and same-name priority), while the previous loose matching is kept as well; strategies are unioned instead of short-circuiting, enforcing "rather over-count references than under-count them" — over-counting only leaves a few more images behind, under-counting deletes images still in use. JSON-escaped slashes (`assets\/a.png`) are also unescaped.
- **Two more blind spots closed: frontmatter and loose text carriers** — Property values such as `cover: "[[a.png]]"` and `cover: "assets/a.png"` are both counted (recursively walking strings, arrays and nested objects; a bare path only counts when it ends in an image extension). `.base` / `.excalidraw` / `.html` / `.txt` files have no fixed link syntax and are scanned with syntax regex plus an image-path fallback, skipping any single file over 2 MB.
- **Stability hardening** — Module-level `g`-flagged regexes reset `lastIndex` before each use, with a zero-width-match guard; a single unreadable or unparsable file no longer aborts the scan; and each file is re-checked against the vault right before deletion to avoid a race where it was moved during the dialog.
