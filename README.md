...
将不可见的空白符号以半透明标记展现，一眼看清文档中的空格、制表符和换行类型：
...
- **隐藏样式**（可折叠） — 6 个开关：加粗、斜体、高亮、删除线、行内代码、**空白符号可视化**
...
- 新增：空白符号可视化 — 空格显示·、制表符显示→、软回车显示↓、回车显示↵
- 新增：设置面板「隐藏样式」新增空白符号可视化开关
...
Renders invisible whitespace characters as semi-transparent markers to reveal the document's structure at a glance:
...
| Soft break (Shift+Enter) | `↓`    | Line break within paragraph     |
| Hard break (Enter)       | `↵`    | Paragraph end                  |
...
- **Style Hider** (collapsible) — 6 toggles: Bold, Italic, Highlight, Strikethrough, Inline Code, **Whitespace Visualization**
...
│   ├── whitespace-visible.ts # Whitespace visualization
...
- New: Whitespace Visualization — render spaces (·), tabs (→), soft breaks (↓), hard breaks (↵) as semi-transparent markers
- New: Settings toggle for whitespace visualization under "Style Hider"
