...
- 修复：聚焦选项开关需重启 Obsidian 才生效 — `focusViewPlugin` 追踪 `lastEnabled` 状态，开关变化时跳过 `selectionSet` 守卫立即重算
...
- **Blank item promotion**: Pressing Enter on an empty list item whose previous line is also empty promotes it one level; top-level items get cleared (if prev line also empty list item, both are cleared at once)
...
- Fix: focus option toggle requires Obsidian restart — `focusViewPlugin` tracks `lastEnabled`, skips `selectionSet` guard on toggle change

**v1.4.3** (2026-06-25)

- Fix: focus mode cursor outside list still matches last item — `computeFoldIndices` uses continuation-line scan instead of `Number.MAX_SAFE_INTEGER`
- Fix: focus mode last item fold range swallows content after list — `computeFoldRanges` last-subtree branch scans for continuations (stops at blank lines) instead of `doc.length`

**v1.4.2** (2026-06-25)

- Optimize: Enter on empty top-level list item also clears previous empty line's format — `changes.from` changed from `line.from` to `prevLine.from`, deleting both lines in one dispatch
...
- Fix: Continuation lines of folded sub-items not folded in focus mode — `computeFoldRanges` scans for soft-break continuations without merging sibling folds
